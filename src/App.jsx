...
export default function App() {
  ...
  const confirmarYGuardar = async (sendWA = false) => {
    setGuardando(true);
    const ahora = new Date();
          ...
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 32 }}>
              ...
              <div className="header-actions" style={{ display: "flex", gap: 12 }}>
                ...
                <button className="btn-outline" style={{ borderColor: "#c8a84c", color: "#7a5c1a", fontSize: 11 }} onClick={() => {
                  const parseCuando = (s) => {
                    if (!s) return 0;
                  ...
                  const ultima = [...reservas].sort((a, b) => parseCuando(b.cuando) - parseCuando(a.cuando)).find(r => r.cuando);
                  if (ultima) abrirEditar(ultima);
                  else showToast("No hay reservas guardadas", "error");
                }}>↩ Última reserva</button>
            ...
            <div style={{ marginTop: 18 }}>
              {reservaEditando ? (
                <div style={{ display: "flex", justifyContent: "flex-end", gap: 10, alignItems: "center" }}>
                  ...
                  <button className="btn-gold" style={{ padding: "8px 20px", fontSize: 11 }} onClick={guardarReserva}>Guardar cambios</button>
                </div>
              ) : (
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {/* Botón WA principal — ancho completo */}
                  <button
                    disabled={guardando}
                    style={{
                      width: "100%", padding: "13px 24px",
                      fontFamily: "'Jost', sans-serif", letterSpacing: 1,
                      textTransform: "uppercase", cursor: guardando ? "not-allowed" : "pointer",
                      borderRadius: 10, background: guardando ? "#a5d6a7" : "#25D366",
                      color: "#fff", border: "none", fontWeight: 700,
                      opacity: guardando ? 0.7 : 1,
                      display: "flex", alignItems: "center", justifyContent: "center", gap: 12,
                      boxShadow: guardando ? "none" : "0 4px 16px rgba(37,211,102,0.35)",
                      transition: "all 0.2s"
                    }}
                    onMouseEnter={e => { if (!guardando) { e.currentTarget.style.background="#1ebe5a"; e.currentTarget.style.boxShadow="0 6px 20px rgba(37,211,102,0.5)"; }}}
                    onMouseLeave={e => { e.currentTarget.style.background=guardando?"#a5d6a7":"#25D366"; e.currentTarget.style.boxShadow=guardando?"none":"0 4px 16px rgba(37,211,102,0.35)"; }}
                    onClick={() => {
                      if (!form.tomadaPor) return showToast("Indica quién toma la reserva", "error");
                      if (!form.nombre || !form.fecha || !form.hora) return showToast("Selecciona nombre, fecha y hora", "error");
                      if (!form.personas || form.personas < 1) return showToast("Indica el número de personas", "error");
                      if (!reservaEditando) {
                        ...
                        const [hh, mm] = form.hora.split(":").map(Number);
                        const esMedio = (hh * 60 + mm) < 20 * 60;
                        if ((esMedio && mediodia) || (!esMedio && noche)) return showToast("No se pueden tomar reservas — día cerrado", "error");
                      }
                      confirmarYGuardar(true);
                    }}>
                    <svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor" style={{ flexShrink: 0 }}><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
                    <span style={{ display: "flex", flexDirection: "column", alignItems: "flex-start", lineHeight: 1.2 }}>
                      <span style={{ fontSize: 16, fontWeight: 800, letterSpacing: 1 }}>{guardando ? "⏳ Guardando..." : "ENVIAR WHATSAPP"}</span>
                      {!guardando && <span style={{ fontSize: 11, fontWeight: 400, opacity: 0.85, letterSpacing: 0.5 }}>y tomar reserva</span>}
                    </span>
                  </button>
                  {/* Segunda fila: Tomar reserva + Cancelar */}
                  <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
                    <button
                      onClick={() => navegarConGuardia(() => setModalAbierto(false))}
                      style={{
                        padding: "9px 18px", fontFamily: "'Jost', sans-serif", fontSize: 11,
                        letterSpacing: 1.5, textTransform: "uppercase", cursor: "pointer",
                        borderRadius: 8, background: "transparent", color: "#6a9a6a",
                        border: "1.5px solid #c8e6c9", fontWeight: 500, transition: "all 0.2s"
                      }}
                      onMouseEnter={e => { e.currentTarget.style.borderColor="#81c784"; e.currentTarget.style.color="#2e7d32"; }}
                      onMouseLeave={e => { e.currentTarget.style.borderColor="#c8e6c9"; e.currentTarget.style.color="#6a9a6a"; }}>
                      Cancelar
                    </button>
                    <button
                      onClick={guardarReserva}
                      style={{
                        padding: "9px 22px", fontFamily: "'Jost', sans-serif", fontSize: 12,
                        letterSpacing: 1.5, textTransform: "uppercase", cursor: "pointer",
                        borderRadius: 8, background: "#2e7d32", color: "#fff",
                        border: "none", fontWeight: 600,
                        boxShadow: "0 2px 8px rgba(46,125,50,0.25)", transition: "all 0.2s"
                      }}
                      onMouseEnter={e => { e.currentTarget.style.background="#1b5e20"; e.currentTarget.style.boxShadow="0 4px 12px rgba(46,125,50,0.4)"; }}
                      onMouseLeave={e => { e.currentTarget.style.background="#2e7d32"; e.currentTarget.style.boxShadow="0 2px 8px rgba(46,125,50,0.25)"; }}>
                      Tomar reserva
                    </button>
                  </div>
                </div>
        ...
        <div className="overlay" style={{ zIndex: 60 }}>
          <div className="modal" style={{ maxWidth: 420, textAlign: "center", padding: "48px 40px" }}>
            ...
            <div style={{ display: "flex", gap: 12, justifyContent: "center" }}>
              ...
              <button className="btn-gold" disabled={guardando} style={{ opacity: guardando ? 0.5 : 1 }} onClick={() => confirmarYGuardar(false)}>
